...
export const run = async ({ baseUrl, sourceDir, availableEnvironments }) => {
  if (!baseUrl) {
  ...
  if (availableEnvironments && !availableEnvironments.includes(getEnvironment(baseViewPage))) {
  ...
  page.on("load", async () => {
    ...
    if (availableEnvironments && environment && !availableEnvironments.includes(environment)) {
